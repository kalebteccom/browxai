...
async function call<T>(handlers: Record<string, (a: unknown) => Promise<{ content: { text: string }[] }>>, name: string, args: Record<string, unknown>): Promise<T> {
...
async function main() {
  for (const k of Object.keys(process.env)) if (k.startsWith("BROWX_") && k !== "BROWX_TRACE_MARKS") delete process.env[k];
  process.env.BROWX_WORKSPACE = mkdtempSync(join(tmpdir(), "browx-trace-cli-"));
  const server = await createServer({ headless: true });
  const handlers = server.handlers as unknown as Record<string, (a: unknown) => Promise<{ content: { text: string }[] }>>;
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const r = await call<Record<string, unknown>>(handlers, "screenshot_marks", {
      session, candidates: [{ ref: refs[0]! }], label: "index",
    });
