...
describe("cache + idb keystone — Phase 7 storage CRUD against real Chromium", () => {
  it(
    ...
    async () => {
      ...
      const entries = await callJson<{ entries: Array<{ url: string }>; count: number }>(
        "caches_list", { session, cacheName: "v1" },
      );
      ...
      const txt = await callJson<{
        found: boolean; kind: string; text: string; status: number; contentType: string;
      ...
      const bin = await callJson<{
        found: boolean; kind: string; contentBase64: string; byteLength: number;
      ...
      await callJson(
        "caches_put",
        { session, cacheName: "v1", url: newUrl, response: { body: "agent-wrote-this", headers: { "content-type": "text/plain" } } },
      );
      const back = await callJson<{ found: boolean; kind: string; text: string }>(
        "caches_get", { session, cacheName: "v1", url: newUrl },
      );
      ...
      const filtered = await callJson<{ count: number; entries: Array<{ url: string }> }>(
        "caches_list", { session, cacheName: "v1", urlPattern: "/cached/" },
      );
      ...
      const del1 = await callJson<{ existed: boolean }>(
        "caches_delete", { session, cacheName: "v1", url: newUrl },
      );
      expect(del1.existed).toBe(true);
      const del2 = await callJson<{ existed: boolean }>(
        "caches_delete", { session, cacheName: "v1", url: newUrl },
      );
      expect(del2.existed).toBe(false);

      // (8) caches_clear — wipes remaining entries, storage stays around.
      const clear = await callJson<{ cleared: number }>(
        "caches_clear", { session, cacheName: "v1" },
      );
      expect(clear.cleared).toBeGreaterThanOrEqual(2);
      const post = await callJson<{ names: string[] }>(
        "caches_list_storages", { session },
      );
      expect(post.names).toContain("v1");

      // (9) caches_delete_storage — drops it; idempotent.
      const drop1 = await callJson<{ existed: boolean }>(
        "caches_delete_storage", { session, cacheName: "v1" },
      );
      expect(drop1.existed).toBe(true);
      const drop2 = await callJson<{ existed: boolean }>(
        "caches_delete_storage", { session, cacheName: "v1" },
      );
  ...
  it(
    ...
    async () => {
      ...
      const dbs = await callJson<{
        supported: boolean; databases: Array<{ name: string; version: number }>;
      ...
      const stores = await callJson<{ stores: string[]; version: number }>(
        "idb_list_stores", { session, dbName: "app" },
      );
      expect(stores.stores).toContain("kv");

      // (3) idb_get — seeded records readable.
      const u1 = await callJson<{ found: boolean; value: { name: string } }>(
        "idb_get", { session, dbName: "app", storeName: "kv", key: "u1" },
      );
      expect(u1.found).toBe(true);
      expect(u1.value).toEqual({ name: "Ada" });

      // (4) idb_get — missing key returns found:false.
      const ghost = await callJson<{ found: boolean }>(
        "idb_get", { session, dbName: "app", storeName: "kv", key: "ghost" },
      );
      ...
      await callJson(
        "idb_put",
        { session, dbName: "app", storeName: "kv", key: "u3", value: { name: "Grace" } },
      );
      const back = await callJson<{ found: boolean; value: { name: string } }>(
        "idb_get", { session, dbName: "app", storeName: "kv", key: "u3" },
      );
      expect(back.found).toBe(true);
      expect(back.value).toEqual({ name: "Grace" });

      // (6) idb_put against a missing store — schema-hint rejection.
      const missing = await callJson<{ ok: boolean; error?: string }>(
        "idb_put",
        { session, dbName: "app", storeName: "absent", key: "k", value: 1 },
      );
      ...
      await callJson(
        "idb_delete",
        { session, dbName: "app", storeName: "kv", key: "u3" },
      );
      const gone = await callJson<{ found: boolean }>(
        "idb_get", { session, dbName: "app", storeName: "kv", key: "u3" },
      );
      ...
      await callJson(
        "idb_clear",
        { session, dbName: "app", storeName: "kv" },
      );
      const r = await callJson<{ found: boolean }>(
        "idb_get", { session, dbName: "app", storeName: "kv", key: "u1" },
      );
  ...
  it(
    ...
    async () => {
      ...
      const cachesRes = await callJson<{ ok: boolean; error?: string }>(
        "caches_list_storages", { session },
      );
      expect(cachesRes.ok).toBe(false);
      expect(cachesRes.error).toMatch(/Navigate the session/);

      const idbRes = await callJson<{ ok: boolean; error?: string }>(
        "idb_list_databases", { session },
      );
