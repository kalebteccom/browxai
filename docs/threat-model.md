...
- `native-device`, default **off**. Tools: none — it gates an ENGINE. `open_session({engine:"ios-app"})` refuses with `capability-required` when it is not granted.

  Native application control on an iOS Simulator (RFC 0008). The gate sits at **session creation** rather than on individual tools, because the posture broadening is the session: opening one boots a simulator, optionally installs a `.app`, launches an application, attaches an XCUITest input pipeline and photographs the screen, all before a single tool call. A per-tool gate would have to be repeated on every tool the engine serves and would still leave session creation ungated.

  **It broadens posture further than any browser capability.** On a simulator it reaches a sandbox. The same code path aimed at a real device — a later phase, explicitly out of scope today — reaches the operator's phone: its photos, its keychain, its logged-in accounts. Treat granting this as granting device control, not browser control.

  **Xcode and WebDriverAgent are operator-supplied**, mirroring the credentials-provider posture: never bundled, never auto-installed, never fetched. browxai shells out to `xcrun simctl` with fixed argv and no shell interpolation (a udid, a bundle id and a deep-link URL are each their own argv element), and speaks HTTP to a WebDriverAgent the operator runs. No npm dependency is added for either, and nothing GPL is reachable — GPL-3.0 `pymobiledevice3` is excluded by the licence floor and is a real-device tunnelling tool this simulator-only engine never needs.

  **A residual secret-leak sink no masking closes.** `fill` uses the driver's element-scoped set-value whenever the element carries an accessibility identifier, so the materialised value never reaches a shell and never reaches the keyboard. Without an identifier it falls back to typing, and the iOS keyboard draws a character-preview bubble above each pressed key — which a screen recording captures **even for a secure field**. The fallback says so in the result's warnings. Add `testID` props in the app to stay on the set-value path.

  **What it cannot observe, it refuses.** The engine declares no `network`, `storage`, `script` or `emulation` sub-interface, so those families return the engine-refusal envelope instead of a plausible empty. There is no protocol-level tap on a native app without a system proxy or a VPN profile, and installing either is the operator's decision, which browxai never makes on their behalf. Loud one-time warning at server boot. Same posture class as `replay` / `network-body` / `secrets`.
