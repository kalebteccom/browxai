...
Root-cause write-ups for tricky bugs, performance traps, and page-side function gotchas: the kind of issue a future maintainer would otherwise hit a second time before remembering it had been solved. Time-ordered. Each entry captures the symptom, the hypothesis chase, the actual root cause, and whatever discipline change (if any) followed.
...
2. If the lesson generalizes, cross-reference it from the closest in-context spot, typically `docs/ai-context/agent-process/code-quality.md` or a pattern doc in the `docs/ai-context/page-side-functions/` tree.
