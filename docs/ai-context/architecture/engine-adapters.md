...
layer and tools call, and that set is satisfied by the handles above.
...
pattern, never by throwing a vague error mid-call.
...
because adb publishes the `localabstract:chrome_devtools_remote` socket. Selecting
...
cleanly and does not duplicate the check.
...
`android`; those checks always run, so the selected engine's readiness, meaning
binary present or device reachable, is visible in one glance). An invalid selection
...
the adapter transparently with no behavior change. A keystone assertion confirms the
...
- **`PlaywrightSnapshotSubstrate` (firefox / webkit)** is the page-side ARIA/DOM
...
(`snapshotSubstrateFor(session)` in `src/page/snapshot-substrate-select.ts`): CDP
...
it wraps, with no per-call allocation (doctrine: hot path measured, not guessed).
...
needs CDP `DOM.getDocument({pierce:true})`, so it is gated off Chromium.
...
(`networkSubstrateFor(session)` in `src/page/network-substrate-select.ts`,
...
- **`CdpNetworkSubstrate` (chromium / android)** owns the `NetworkBuffer` /
  ...
  below).
- **`PlaywrightNetworkSubstrate` (firefox / webkit)** is `PlaywrightNetworkBuffer`
...
`goto`+settle. Pure tap `open()`+`close()` overhead is **CDP 0.22 ms vs event
...
- **timing precision**: `ms` is wall-clock (request-seen → response-seen), the
  same approximation the CDP tap uses, with no high-resolution `timing()` deltas.
...
- **SSE**: Server-Sent-Events arrive as a long-lived `eventsource` response rather
  than a discrete Playwright event, so the SSE half of `ws_read` degrades off Chromium
...
and ephemeral launch shapes over `resolveBrowserType("webkit")` (Playwright's
bundled WebKit build: the **WebKit-ENGINE correctness lane** per RFC D7, **not**
Safari; a real-Safari surface is a separate, tiered companion product, never a
browxai engine adapter). It mirrors `PlaywrightFirefoxAdapter` and mints **no
...
nine cross-browser sub-interfaces + `deep: false`, and `webkit` is in
...
safaridriver hard-isolates automation into a clean ephemeral window, so
attach-to-the-live-session is impossible by design.
...
- the substrate selectors (`snapshotSubstrateFor` / `networkSubstrateFor`) key on
  ...
  `CdpNetworkSubstrate` path as desktop Chromium**, verbatim and automatically;
...
removes the adb forward; it never closes the browser (not-owned, since it is the
user's phone Chrome).
...
Firefox and WebKit run the snapshot/a11y read + action core, the network/WS tap
and response-body fetch through the engine-agnostic walker + network substrates
and the capability-based gate. **Android is the standout: every row is `works`,
...
Android-specific limit is launch-shape: managed and ephemeral launch refuse with
`android-launch-not-supported`, because Android is attach-only.
...
(`deep: false`), so no per-tool work brings it online, only the adapter and the
capability row. **The Android column is the inverse of WebKit's: it is identical
to _Chromium_ by construction.** Android declares `deep: true`, so the CDP
substrates and the capability-based gate route it through the exact Chromium path
with no per-tool work, only the adapter plus the adb plumbing plus the capability
row. The device-gated keystone asserts the engine tag, navigate → snapshot →
find, AND a deep tool (`coverage_start`) running, the proof of `deep: true`, on a
real connected device (skips cleanly otherwise). The attach-only `n/a` for
...
**subset** (`lifecycle/navigation/snapshot/input/storage/script/capture`,
`deep: false`; no `network`, no full `emulation`). Per-family status:
...
- **gated (page()-throw; an action substrate over WebDriver element interaction is
  ...
  `eval_js` (each needs a small safari handler branch, a follow-up).
