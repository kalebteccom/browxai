...
function, custom lint rule, dependency-cruiser rule, budget, and CI gate — what it
proves, where it lives, how to run it, and which law (L1–L10) it enforces. The
...
| `interface-member-budget`      | Each segregated `ToolHost` sub-port stays under the member ceiling; `ToolHost` declares zero own members (it is the pure intersection — no god-object re-forms).                                                    | `interface-member-budget.test.ts`      | L4       |
...
> decision at the loop_ rather than blocking the build
