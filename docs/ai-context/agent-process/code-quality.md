...
The bar every plan, review, and implementation aims for: elegance and pragmatism over speed and convenience. A small delay is worth it when the codebase ends up better than you found it.
...
Rationale: a terse, scannable log. Ticket, iteration and plan references rot, so state the why in the PR description or in `docs/ai-context/`. The commit body stays empty.
...
Always suggest an alternative. Describe the better approach and say why it's better.

## Prefer elegant solutions

Choose the simplest correct solution. Avoid over-engineering, and avoid lazy shortcuts that create tech debt. When multiple valid approaches exist, present the alternatives with trade-offs and recommend one. If implementing a feature the right way takes slightly longer but produces meaningfully better code, take the right way and note the trade-off.

## Verify, don't assume

Read the actual file before naming APIs, imports, config keys, generated fields, schemas, or deployment behavior. Plan snippets, memory, and old review notes can be stale. Hallucinated APIs, paths and signatures are a recurring failure mode. Verifying takes seconds. Debugging a hallucinated reference takes hours.
...
- Don't reference the current task / fix / callers ("used by X", "added for the Y flow", "handles the case from PR #123"). All of that belongs in the PR description.
- No internal tracking identifiers in code or comments. Ticket / plan / iteration / PR refs (`W-X#`, `R[Oo]und-\d+`, `ask #N`, `TICKET-N`, `JIRA-N`, `#1234`, `ROLLBACK-SAFETY-PLAN`, `Security-RECOMMENDED-1`, etc.) are project-management artifacts, not code context. They rot, they mean nothing to a future reader, and they belong in the commit/PR body. State the actual reason instead: write _why_ the code is the way it is, not _which ticket asked for it_. Example: `// Kept as a zombie for safe rollback; remove in a follow-up cleanup`, never `// Kept per ROLLBACK-SAFETY-PLAN Rule 1`.
- One exception: load-bearing identifier schemes like `INV-N` invariant tags, whose literal text an enforcing test discovers. An identifier a test reads is code, not provenance, and stays.
...
Internal working docs under `docs/ai-context/` and `docs/rfcs/` are exempt. Phase and decision history are legitimate context there. Anything a user or calling agent can read must be clean.
...
- `src/server.ts` is registry composition only, with no business logic. It wires the tool registry, composes gates, and exposes the MCP surface.
...
- `BrowxaiClientWithPlugins<Schema>` extends a narrow `BrowxaiClient` base via the SDK type-gen seam, so adopters who don't use plugins never see the plugin surface.
...
- The plugin runtime's `PluginApi` interface is dependency-inverted by design: plugins call `api.callTool(...)` and `api.registerTool(...)`, and never reach into browxai internals.

## Architecture enforcement: the automated guardrails

The SOLID section above states the _rules_. This section states the macro
guardrails: the machines that fail when a rule is broken. Every guardrail below
names its machine. A rule with no machine is a known gap, not a free pass, and a
guardrail that says "code-review only" is exactly such a gap. Macro guardrails
carry the same enforcement weight as the micro rules on comments, naming and
async safety. The process for adding and amending them lives in
[RFC 0004](../../rfcs/0004-architecture-hardening.md).
[`../architecture/fitness-functions.md`](../architecture/fitness-functions.md)
indexes every fitness function: what each proves, which law it enforces, and the
finding it closes.
...
| 9   | **Bounded everything.** Every ring / deadline / poll window / recursion depth has an explicit, tested bound.                                                              | L7         | `bounded-resource` fitness test (`error`) + the `bounded-resource` lint rule (advisory `warn`, a heuristic)           | `pnpm test` (+ `pnpm lint` warn)               |
...
rationale, never an inline `eslint-disable` and never a one-off budget bump in a
...
(tightening) as modules shrink is always welcome and needs no amendment. One rule
ships advisory and never blocks, the `bounded-resource` lint heuristic (#9). A
linter cannot prove termination, so it prompts at the loop; the bound's proof is
the `bounded-resource` _test_ and the `invariant()` termination check.
...
It is the macro-rule equivalent of `tracker-id-auditor`.
...
A new plugin ships a package.json with a browxai manifest, `schema.d.ts`, `src/index.ts` with `register(api)`, a README, a LICENSE, tests, and a CHANGELOG row in the host changelog.
