...
`engine: "ios-app"` drives a native iOS application on the **Simulator** through its XCUITest accessibility hierarchy. It is not a browser: there is no document, no URL, no cookie jar and no console. Real devices are out of scope — they need code signing, a provisioning profile, a WebDriverAgent build and attached hardware.

**It requires the off-by-default `native-device` capability.** The gate is at session creation, so `open_session({engine:"ios-app"})` refuses with `capability-required` before anything is booted, installed or launched. See `docs/threat-model.md`.
...
**Not yet:** segmented video capture and the `native/*` replay event types (RFC 0008 P3), `console_read` over the simulator syslog, and `app_launch` / `app_terminate` / `app_reset` / `device_list` as MCP tools — lifecycle today runs at session creation from the environment above.
