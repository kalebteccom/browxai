...
- Supported permission names (v1, 12 total): `camera`, `microphone`, `geolocation`, `notifications`, `clipboard-read`, `clipboard-write`, `midi`, `midi-sysex`, `payment-handler`, `background-sync`, `accelerometer`, `gyroscope`, `magnetometer`. USB / Bluetooth / HID are out of scope for v1 (slated for a future `device-emulation` capability).
