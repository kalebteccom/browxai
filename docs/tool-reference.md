...
**`capabilities` can only narrow.** `BROWX_CAPABILITIES` (or the built-in default set when it is unset) is the ceiling. `set_config` refuses a `capabilities` patch that names anything outside the active set with `{ ok:false, error:"capabilities-not-widenable", widening:[…] }`, and at every server start a saved or session `capabilities` list is intersected with the ceiling, so a saved list can drop capabilities and never add them. Saved names outside the ceiling are ignored with a startup warning. Enabling a capability is an edit to the server's environment and a restart.
...
Blocks the calling agent until the human responds. The `prompt` is logged to stderr. The operator answers from DevTools after switching the console's context dropdown from `top` to `browxai`, an isolated JS world the page's own scripts cannot reach. The supported kinds:
...
- **In the `browxai` isolated world** (the answering one). browxai creates the world over CDP on every page the session owns, evaluates the helper in it, and installs a per-session CDP binding scoped to that world alone. A call is accepted only when CDP reports it came from one of those world contexts. Re-created on every navigation.
- **In the page's main world** (`window.__browx`, display-only). Every method logs where the real channel lives and returns `false`; `status()` returns `{ state: "display-only", humanWorld: "browxai" }`. Page scripts can call, replace or delete it without effect on the server. The `data-browx-signal` attribute is no longer read.
...
__browx = {
  ...
  respond(value) / confirm(yes) / choose(idx) / input(text),
