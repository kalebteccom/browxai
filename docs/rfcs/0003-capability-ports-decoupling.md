...
**Date:** 2026-06-13
**Status:** Draft. **P1–P3 LANDED & gate-green** (1826 unit + the 5-engine keystones). Seven capability ports now: `SnapshotSubstrate`, `NetworkSubstrate`, `ActionSubstrate`, `CaptureSubstrate`, `StorageSubstrate` (cookies + web-storage), `ScriptSubstrate`, `EmulationSubstrate`. The action / screenshot / cookies / web-storage / eval / geolocation-colour-motion handlers are all engine-blind. The scattered `if (engine === "safari")` handler branches went from **11 → 1**: the engine check now lives only in the five `*For(e)` capability selectors (one per port family), plus one residual in `open_session` (a P4-sweep item). **`StorageSubstrate` also now covers IndexedDB + the Cache API** (Playwright path moved verbatim; Safari refuses cleanly, because both APIs are async and safaridriver's `execute/sync` is synchronous with no async-script client, so a refusal is the correct contract, not a broken impl). The Safari keystone proves navigate/click/fill/snapshot/find/screenshot/cookies/**localStorage** on real Safari. The engine check now lives in exactly 6 sites: the five `*For(e)` capability selectors (one per family) + one residual `open_session` URL read. Remaining: verify/frames/workers/har/downloads/dom-export/etc. tool families (mechanical extension of the proven pattern), then P5 project-wide. **Author:** Claude (architecture).
...
- **32** `requireCdp()` sites, the **correctly-gated** CDP-deep tools (perf/coverage/heap/…); they genuinely require CDP and refuse on engines without it. That is the honest-gating pattern, **not** a coupling smell, and is out of scope here.
