...
**Date:** 2026-06-13
**Status:** Draft — **P1 (`ActionSubstrate`) + P2 (`CaptureSubstrate` / `StorageSubstrate` / `ScriptSubstrate`) LANDED & gate-green** (1805 unit + the 5-engine keystones; the action / screenshot / cookies / eval handlers are all engine-blind now). The scattered `if (engine === "safari")` handler branches went from **11 → 1** — the engine check now lives only in the four capability selectors (`actionsFor`/`captureFor`/`storageFor`/`scriptFor`), exactly one per port, plus one residual in `open_session` (a P4-sweep item). P3–P5 remain. **Author:** Claude (architecture).
