...
## 3. `code-quality.md`: add the "Architecture enforcement" section
...
> of every fitness function, what each proves and which law it enforces, is
...
>
> The architecture lane is **static and fast**, doing AST / dependency-graph / string
> analysis with no browser, and it runs in `pnpm test`, not the keystone lane. The one
...
> `registerXxxTools(host)` line in `server.ts`, so composition stays one line longer,
> the existing families are untouched.
>
> `ToolHost` is a god-object today (35 members) and the engine-selected substrate
> selectors live in `host-build.ts`, and both are targets of RFC 0004's segregation
...
> | [0004](0004-architecture-hardening.md) | Architecture hardening: a safety-critical maintainability standard (SOLID/OCP refactor + fitness-function guardrails + AI-doc/harness enforcement) | Draft, a proposal; evidence + specs in [references/](references/) (`0004-01`…`0004-09`). |
...
>   one registry registration, and **never** an edit to `src/session/{managed,incognito,byob}.ts`
...
The agent's relationship to the existing roster is deliberate: `capability-gate-auditor` checks the *capability lattice* (off-by-default, threat-model row, keystone denial) and `architecture-fitness-auditor` checks the *structural seams* (OCP, layering, budgets). They overlap on one cell, in that both care whether a handler inlines a gate, and that overlap is the point: it is the single most-cited SRP defect, worth two agents watching.
...
> seam, and find the registry the change should have used.
...
- **The fitness lane is frozen before the refactor starts.** P0 lands the suite as
  ...
  its phase pass by loosening a fitness function, so the reviewer treats any change to
...
>       fitness row is a seam with no guard, so the change is half-done.
...
| `code-quality.md` | New "Architecture enforcement" section (8 guardrails × command); the meta-rule; the `architecture-fitness-auditor` note. | *"A future contributor reading code-quality.md will NOT learn the guardrails."* |
...
Every edit is text only, with no `src/` change, consistent with the parent RFC's non-goals. The edits land in P5 of the refactor plan ([`0004-04`](../../rfcs/references/0004-04-refactor-plan.md)), *after* the suite they document exists, so no doc points at a check that is not yet runnable.

---

## Related

- [`0004-architecture-hardening.md`](../0004-architecture-hardening.md) is the parent RFC; D12 is this reference's charter, §4a/the laws its content.
