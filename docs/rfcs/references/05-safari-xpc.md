...
- `com.apple.private.security.storage.WebDriver` is a data-vault storage class isolating WebDriver state under `~/Library/WebDriver` (backs the clean-slate guarantee, §1.6).
...
2. **Sandbox / hardened runtime.** `sandbox_check("mach-lookup", "com.apple.webinspector")`; App Sandbox blocks a global Mach lookup unless the app carries a temporary-exception entitlement for it.
3. **SIP** protects the Apple daemon, so the iOS trick of injecting into `webinspectord` to bypass the check has no macOS equivalent.
...
- **For `com.apple.webinspector` specifically:** browxai cannot present `com.apple.webinspector.allow` (restricted, un-grantable) or `com.apple.private.webinspector.driver-client` (private). The supported lever is `isInspectable` **on targets you control**, which is useless for the user's Safari.
...
- **Ceiling:** page-context JS sandbox only, with no privileged browser API, no protocol-level network interception, **closed shadow DOM is invisible**, **strict CSP blocks injected JS**, per-document.
...
- **Stability/policy:** decades-old API but TCC tightened across recent macOS; App Store scrutinizes `CGEvent.post`. Highest brittleness, a genuine fallback.
...
| **SafariWatir, legacy `.safariextz`** | AppleScript / old Selenium extension | n/a | Dead |
...
| **Direct `webinspectord` / `com.apple.webinspector`** | ✅ (full CDP-grade *if* connected) | ✅ | ✅ | ✅ | ✅ | Yes *if* connected | n/a | n/a | **❌ BLOCKED: private entitlement `com.apple.private.webinspector.driver-client`; CoreTrust/AMFI + peer validation** |
