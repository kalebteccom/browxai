...
**(b) Safety-critical origin.** Power-of-Ten rules 2 and 3: *every loop has a fixed upper bound a checker can statically prove*, and *no dynamic memory allocation after initialization*. The reason is exact: an unbounded loop or an unbounded allocation is a latent denial-of-service against the vehicle itself, since a single oversized input exhausts the resource and wedges the system at the worst possible time. The bound is not a tuning knob; it is a safety property.
