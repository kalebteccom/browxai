# WebDriver BiDi and adjacent browser-automation standards: state of the world, June 2026
...
- Where it ships first: **WebKitGTK/WPE**, with BiDi enabled as an *experimental feature* in WebKitGTK **2.47.4 (5 Feb 2025)** ([webkitgtk.org release notes](https://www.webkitgtk.org/2025/02/05/webkitgtk2.47.4-released.html)); Linux `WebKitWebDriver` is the vehicle.
...
- **Brave**: keeps CDP/`--remote-debugging-port` functioning (with Chrome's 136+ user-data-dir restriction inherited); drivable by version-matched chromedriver+mapper. No first-party BiDi statements (confidence: medium, on absence of evidence).
...
### 5.1 Chromium and its derivatives (Chrome, Edge, Brave, Arc…)
...
- **The best standards-native BYOB story.** Start Firefox with `--remote-debugging-port[=9222]` on *any* profile, including the user's real one and with no Chrome-136-style profile ban, and the Remote Agent serves a **WebDriver BiDi WebSocket** (CDP gone since Fx 141).
...
- **You can never attach to the live user session. Period.** Apple's model ([webkit.org "WebDriver is Coming to Safari in iOS 13"](https://webkit.org/blog/9395/webdriver-is-coming-to-safari-in-ios-13/), [Apple: Testing with WebDriver in Safari](https://developer.apple.com/documentation/webkit/testing-with-webdriver-in-safari)):
  ...
  - Sessions run in **dedicated automation windows**: "an automated browser always starts from a clean state like a private browsing session… doesn't have access to Safari's browsing history, AutoFill data"; an orange Smart Search field marks them; local state (cookies etc.) is destroyed at session end.
...
- Classic: a remote end has at most one active session (per spec), which guarantees exclusivity and kills multi-client by design.
...
2. **Firefox support is cheap now**: a BiDi client against `--remote-debugging-port` gives launch-or-glassbox attach on real profiles, preload scripts, network interception incl. bodies, emulation, with no geckodriver needed for BiDi-only sessions. Mind the loopback/allowlist flags and `navigator.webdriver=true`.
