...
**Date:** 2026-06-13 · **Status:** IN PROGRESS — **Increments 1, 2, 3a, 3b, and 3c-network LANDED & gate-green** (1767 unit tests). Done: transport clients, adapter, engine declaration, snapshot substrate, substrate selection, `buildSafariSession` no-Page seam, incognito/byob refusals, and the no-op network substrate. **NEXT = 3c-server (the `server.ts` guard map below — mechanical) + 3c-tools (safari handler branches) → 3d flip `IMPLEMENTED_ENGINES` → 3e keystone (validates on real Safari).** `safari` is still NOT operator-reachable. **Author:** Claude (probe + spike + validated plan). **Read this first if you are picking up the real-Safari engine.** It is the single source for continuing the `SafaridriverHybridAdapter` build. Pairs with [`06-safari-bidi-probe.md`](06-safari-bidi-probe.md) (the empirical capability evidence) and the RFC's D7 / line-20 / P4 (corrected by this workstream).
