...
**Date:** 2026-06-13 · **Status:** ✅ **SAFARI IS OPERATOR-REACHABLE & KEYSTONE-VALIDATED ON REAL SAFARI.** Increments 1, 2, 3 (a/b/c-network/c-server/c-tools/d/e) ALL LANDED & gate-green (1771 unit + the 3-test real-Safari keystone passing on this Mac). `browxai --engine safari` / `BROWX_ENGINE=safari` opens a real Safari session: navigate (WebDriver Classic), snapshot/find (DOM-walk over execute/sync), deep tools refuse with `engine:safari`. The adapter even negotiated BiDi (`hasBidi:true`). **ALL 5 INCREMENTS LANDED.** Inc 4 (BiDi `log.entryAdded` → `console_read` via `ConsoleBuffer.ingest`) + Inc 5 (server guards + deep/network gating + engine-adapters.md capability matrix + Safari section) are done. **REMAINING (follow-ups, NOT blocking, each a focused unit):** the click/fill **action substrate** over WebDriver element interaction (click/fill currently self-gate via the honest `page()`-throw; see §6 Inc-3 "3c-tools"); `screenshot`/`cookies_*`/`eval_js` safari handler branches; a nicer curated central gate than the page()-throw (a `registry.peek`-based allowlist in the `register` wrapper). **Author:** Claude (probe + spike + validated plan). **Read this first if you are picking up the real-Safari engine.** It is the single source for continuing the `SafaridriverHybridAdapter` build. Pairs with [`06-safari-bidi-probe.md`](06-safari-bidi-probe.md) (the empirical capability evidence) and the RFC's D7 / line-20 / P4 (corrected by this workstream).

---

## 0. TL;DR for the next agent

- **What this is:** add real Safari (`safaridriver`, non-BYOB isolated automation windows) as the **5th** `BrowserEngine` adapter and the **FIRST non-Playwright adapter**. Classic WebDriver is the workhorse; experimental BiDi is the additive bidirectional layer.
...
- **The hard part (NOT a normal adapter):** the `BrowserEngine` port and the whole session/server layer are built on a Playwright `Page`. Safari has none. This is a **session-layer seam**, not just a new adapter file. See §3 for why it's the biggest engine add.
...
Add to `src/engine/capabilities.ts` (mirroring the others). Safari serves a **curated subset**, and this is the honest sub-interface set:
...
Implementation: `src/page/snapshot-substrate-safari.ts` holds `SafariClassicSnapshotSubstrate implements SnapshotSubstrate` (interface at `src/page/snapshot-substrate.ts:46`). Mirror `PlaywrightSnapshotSubstrate` (same file, line 102) but replace `runDomWalkOnFrame(this.page.mainFrame(), …)` with a call that ships `PAGE_SCRIPT` through `SafariWebDriverClient.executeScript`. Reuse `mergeDomWalkIntoTree` + `elementKey` + `annotateStructuralContext` verbatim, so refs stay stable across substrates. Wire `src/page/snapshot-substrate-select.ts` (`snapshotSubstrateFor`) to pick the Safari substrate for engine `safari` (it currently keys on CDP presence → would fall to the Playwright walker, which needs a Page Safari lacks; add an explicit safari branch).
...
8. `test/keystone/safari.keystone.test.ts`: real-Safari keystone. Sync availability probe at module load (`statSync('/usr/bin/safaridriver')` + macOS check) + `describe.skip` otherwise (green on Linux/CI; mirrors webkit/firefox/android keystones). When present: `createServer({browserType:"safari"})` → open_session → assert `list_sessions.engine==="safari"`; navigate → snapshot → find → fill → click (Classic substrate); cookies + screenshot (Classic); `console_read` surfacing a `log.entryAdded` line (BiDi); and assert the gated families structured-refuse with `engine:"safari"`.
   - **PLUS (verifier addition): the action seam**: `actions.ts`/`actionresult.ts`/`locator.ts` need a non-Playwright path. Either an `ActionSubstrate` seam (parallel to `SnapshotSubstrate`) or a Safari action-window that derives nav lifecycle from BiDi events / Classic url-poll instead of `page.on('framenavigated')`. **This is bigger than the snapshot port and was UNDER-scoped in the original plan, so budget for it.**
...
- **Inc 3 / task #33: the no-Page session seam + action seam (THE BIG ONE). PARTIALLY LANDED.**

  **✅ 3a (LANDED):** `BrowserSession.safari?(): SafariSessionHandle` added (session/types.ts); `snapshotSubstrateFor` (snapshot-substrate-select.ts) now picks `SafariClassicSnapshotSubstrate` for safari BEFORE the `page()` branch, bridging the handle's `execute/sync`+`currentUrl`; `SubstrateCapableSession` widened. Unit-tested.

  **✅ 3b (LANDED):** `src/session/safari-session.ts`: `buildSafariSession(handle)` returns the BrowserSession whose `page()` throws `NO_PLAYWRIGHT_PAGE`, `safari()` returns the handle, `close()` tears down. `managed.ts` has the safari early-return branch (`new SafaridriverHybridAdapter().launchManaged()` → `buildSafariSession`). `incognito.ts` refuses safari (`safari-incognito-not-supported`); `byob.ts` refuses safari (`safari-attach-not-supported`) + the stale "Safari has not shipped BiDi" comment is fixed. Unit-tested (the refusals + buildSafariSession; the managed branch is keystone-tested once reachable).

  **⏭️ 3c (NEXT, the large/risky piece): `server.ts` session-creation integration.** The block at **`server.ts:~837–1017+`** is ~180 lines ALL built on `sess.page().context()`: `applyHarReplay`, `new ConsoleBuffer().attach(sess.page())`, `networkSubstrateFor(sess)` (`.attach()`), `SecretRegistry`, `BrowxBridge.attach`, `DialogPolicyState`+`attachDialogPolicy`, `PermissionPolicyState`+`attachPermissionPolicy`+`applyPermissionCdpBaseline`, `NotificationPolicyState`, `FsPickerPolicyState`, `DownloadsRegistry`+`attachDownloadCapture`, `ArtifactsRegistry`, `applyOverlayHide`, `applyStealth`, `newEmulationState`/`WebDeviceEmulationState`+`attachDeviceEmulation`, and the `context().on("page", …newCDPSession…)` reapply. **Every line throws on Safari.** These vars (`consoleBuf`, `networkBuf`/`wsBuf`, `secretsReg`, `br`, the policy states, `downloadsReg`, `artifactsReg`, `deviceEmulation`, `webDeviceEmulation`) are **captured by the downstream tool-dispatch context**, so they need Safari stand-ins. **RECOMMENDED APPROACH (lower risk than guarding the shared block): an early `if (sess.engine === "safari") { … }` branch that builds a Safari-specific session context**: `snapshotSubstrateFor(sess)` (works now), a `ConsoleBuffer` fed from BiDi `log.entryAdded` (Inc 4) or empty, an **empty/gated** network buffer, a real `SecretRegistry`/`ArtifactsRegistry`, and **no** BrowxBridge/policies/CDP/device-emulation (gate the tools that need them), then returns/continues to the same handler-registration with the Safari context. Read how the block's vars flow into the per-session handler context (the code AFTER line ~1017) before choosing branch-vs-guard. **Also the action path:** `navigate`/`click`/`fill` route through `actions.ts`→`runInActionWindow`(`actionresult.ts`)→`ctx.page`, all Playwright. For Safari, either (a) gate click/fill/navigate-via-envelope initially and ship the READ subset (snapshot/find/screenshot/cookies/eval) first, or (b) add a Safari action path that uses `handle.webDriver` (`findElement(cssPath)`→`elementClick`/`elementValue`; `navigate` via `webDriver.navigate`) and a lightweight Safari ActionResult. **Recommend (a) for the first reachable landing**, (b) as a follow-up.

  **✅ 3c-network (LANDED):** `SafariNoopNetworkSubstrate` (network-substrate.ts) + `networkSubstrateFor` safari branch (network-substrate-select.ts): empty rings, zero-traffic action tap, structured `network_body` refusal. So the session-creation `networkSubstrateFor(sess)`/`.attach()`/`.setSecrets()` calls are already safari-safe.

  **⏭️ 3c-server (NEXT, mechanical, derived guard map). Two key findings:** (1) **`engineGate` is only on the 27 DEEP_TOOLS** (grep confirmed); every other handler uses `sess.page()` directly. (2) **The `page()` throw IS the refusal mechanism**: `buildSafariSession.page()` throws `NO_PLAYWRIGHT_PAGE` (a structured message), and the SDK's `registerTool` wrapping (server.ts:`register` at ~1518-1535) turns a handler throw into an error result, so unsupported tools self-gate with a named error on safari sessions ONLY (contained; no other engine affected). A nicer curated refusal can be a follow-up via a `registry.peek(sessionId)`-based gate in the `register` wrapper (peek is non-creating, registry.ts:329) + a `SAFARI_SUPPORTED_TOOLS` allowlist, but is NOT required for a first landing.

  **The guard map** (wrap each page-touching CALL in `if (sess.engine !== "safari") { … }`. Keep the page-free `const` declarations unconditional; the guard is always-true for the other 4 engines → byte-identical). In the session-creation block (server.ts ~837-1017): `applyHarReplay` (838, add `&& sess.engine!=="safari"`), `consoleBuf.attach(sess.page())` (841), `br.attach(sess.page().context())` (859), `attachDialogPolicy` (865), `attachPermissionPolicy`+`applyPermissionCdpBaseline` (876-895, one guard), `attachNotificationPolicy` (908-920), `attachFsPickerPolicy` (932-955), `attachDownloadCapture` (964), `applyOverlayHide` (974), the `stealth` block (979-985, add engine check to the `if`), `attachDeviceEmulation`+the `context().on("page", …newCDPSession…)` (1001-1017, one guard). In the return object: the `wsInteractive` IIFE `reg.install(sess.page())` (1055) and the `workers` IIFE `reg.installPageWrapper(sess.page())` (1069); guard each install. In teardown (1098-1134): `e.perf.closeIfRunning(requireCdp(e.session))` (1102) + `e.coverage.closeIfRunning(...)` (1106): guard with `if (e.session.engine !== "safari")` (requireCdp throws synchronously for safari, BEFORE the `.catch`). Also guard the HAR/video state activation: `harState.active=true…` (server.ts:811) and `videoState.active=true…` (server.ts:825) must each gain `&& sess.engine !== "safari"`, otherwise a safari session opened with `recordVideo`/`recordHar` sets `videoState.active`, and teardown's `e.video.active ? e.session.page() : …` (1121) then calls `page()` and throws. (The keystone won't request video/har so it stays false in practice, but guard it for correctness.) `snapshotSubstrateFor(sess)` (1028) + `networkSubstrateFor(sess)` (848) are ALREADY safari-safe, so leave them.

  **⏭️ 3c-tools (supported subset needing safari handler branches):** `navigate`/`go_back`/`go_forward` (→ `handle.webDriver.navigate` instead of the Playwright action envelope), `screenshot` (→ `handle.webDriver.screenshot`), the `cookies_*` family (→ `handle.webDriver.getCookies`/`addCookie`), `eval_js` (→ `handle.webDriver.executeScript`). `snapshot`/`find`/`text_search`/`extract` use `ctx.snapshotSubstrate`; VERIFY they don't also touch `sess.page()` (if clean, they work with no branch). click/fill/etc. need the action substrate, so gate (page()-throw) for the first landing.

  **⏭️ 3d:** flip `"safari"` into `IMPLEMENTED_ENGINES` (select.ts:26); update `select-operator.test.ts` (lines 16/30/38 engine-list + 84-91 safari-throws→resolves) + `engine.test.ts` "not operator-reachable" test. **⏭️ 3e:** the keystone (`createServer({browserType:"safari"})`, skip-when-absent via statSync+darwin; opens REAL Safari windows on this Mac. It is the end-to-end validator that catches any missed `page()` guard. Assert: open → navigate → snapshot → find; screenshot; cookies; and that a gated tool returns the safari error).
  ...
  **Code-confirmed integration design (verified against `managed.ts` + `session/types.ts` @ Inc-2 main):**
  ...
  7. **Keystone (file 8):** `createServer({browserType:"safari"})` end-to-end; skip-when-absent (statSync + darwin). It opens REAL Safari windows, so it only runs on a mac that has Safari installed.
...
2. **One-session-at-a-time**: adapter-level reject-2nd vs registry-level serialize (`registry.ts:289`). Decide before Inc 3, because the two options put the refusal in different layers.
...
5. **No headless**: the keystone and any managed session open VISIBLE Safari windows. Skip-without-Safari covers Linux CI; a mac CI runner would pop real windows.
