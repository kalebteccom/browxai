...
**Date:** 2026-06-13 · **Status:** IN PROGRESS — docs landed; **Increments 1, 2, and 3a+3b LANDED & gate-green** (1764 unit tests). Done: transport clients, adapter, engine declaration, snapshot substrate, substrate selection for safari, the `buildSafariSession` no-Page seam, and incognito/byob safari refusals. **NEXT = Inc 3c: the `server.ts` session-creation integration (the large/risky piece) → then flip `safari` into `IMPLEMENTED_ENGINES` (3d) + keystone (3e).** `safari` is still NOT operator-reachable. **Author:** Claude (probe + spike + validated plan). **Read this first if you are picking up the real-Safari engine.** It is the single source for continuing the `SafaridriverHybridAdapter` build. Pairs with [`06-safari-bidi-probe.md`](06-safari-bidi-probe.md) (the empirical capability evidence) and the RFC's D7 / line-20 / P4 (corrected by this workstream).
...
- **Inc 3 / task #33 — the no-Page session seam + action seam (THE BIG ONE). PARTIALLY LANDED.**
  ...
  **⏭️ 3d:** flip `"safari"` into `IMPLEMENTED_ENGINES` (select.ts:26); update `select-operator.test.ts` (lines 16/30/38 engine-list + 84-91 safari-throws→resolves) + `engine.test.ts` "not operator-reachable" test. **⏭️ 3e:** the keystone (`createServer({browserType:"safari"})`, skip-when-absent; opens REAL Safari windows on this Mac).
