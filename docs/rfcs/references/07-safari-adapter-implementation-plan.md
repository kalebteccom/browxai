...
**Date:** 2026-06-13 · **Status:** ✅ **SAFARI IS OPERATOR-REACHABLE & KEYSTONE-VALIDATED ON REAL SAFARI.** Increments 1, 2, 3 (a/b/c-network/c-server/c-tools/d/e) ALL LANDED & gate-green (1771 unit + the 3-test real-Safari keystone passing on this Mac). `browxai --engine safari` / `BROWX_ENGINE=safari` opens a real Safari session: navigate (WebDriver Classic), snapshot/find (DOM-walk over execute/sync), deep tools refuse with `engine:safari`. The adapter even negotiated BiDi (`hasBidi:true`). **REMAINING (follow-ups, NOT blocking reachability):** Inc 4 (wire BiDi `log.entryAdded` → `console_read`); the click/fill **action substrate** (currently self-gate via the `page()`-throw — a real follow-up, see §6 Inc-3 "3c-tools"); screenshot/cookies/eval safari handler branches; Inc 5 (engine-adapters.md capability matrix row + a nicer curated central gate than the page()-throw). **Author:** Claude (probe + spike + validated plan). **Read this first if you are picking up the real-Safari engine.** It is the single source for continuing the `SafaridriverHybridAdapter` build. Pairs with [`06-safari-bidi-probe.md`](06-safari-bidi-probe.md) (the empirical capability evidence) and the RFC's D7 / line-20 / P4 (corrected by this workstream).
