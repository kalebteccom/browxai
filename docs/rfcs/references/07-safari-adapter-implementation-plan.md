...
**Date:** 2026-06-13 · **Status:** IN PROGRESS — docs landed, code build not started. **Author:** Claude (probe + spike + validated plan). **Read this first if you are picking up the real-Safari engine.** It is the single source for continuing the `SafaridriverHybridAdapter` build. Pairs with [`06-safari-bidi-probe.md`](06-safari-bidi-probe.md) (the empirical capability evidence) and the RFC's D7 / line-20 / P4 (corrected by this workstream).
...
- **Inc 1 / task #31 — transport + adapter + engine declaration (unit-only, NOT operator-reachable).** Build files 1–4 + 6–7 above + `SAFARI_CAPABILITIES` + `capabilitiesFor('safari')` + `EngineKind` adds `safari`. Do NOT add safari to `IMPLEMENTED_ENGINES` yet. Gate: typecheck/unit/lint/format/build/docs:build (no keystone needed). This is the safe first commit — proves the transport + adapter + declaration with mocks.
- **Inc 2 / task #32 — snapshot substrate.** File 5 + snapshot-substrate-select branch + unit test. Spike-proven (§4).
