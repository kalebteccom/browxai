...
**Behavior-preservation.** Trivial: no `src/` change. The full unit suite + the five-engine keystone lanes (`headless`, `firefox`, `webkit`, `android`, `safari`, in `test/keystone/*.keystone.test.ts`) run unchanged as proof that adding the harness perturbed nothing.
...
After (P1, where the engine owns its own post-wire and the registry has no engine literals):
...
- `src/tools/session-registry.ts` + the session types: `SessionEntry` (the 40-field god object) is segregated into the role-bundles its consumers use (engine-core vs policy vs feature concerns), per the session `topRefactors` #2, so tools depend on the sub-object, not the fat record.

**Files edited / created: family extractions (D4, DRY).**

- The five policy classes (dialog / permission / notification / fs-picker / device-emu) share an identical buffer+cap+record pattern. Extract `src/session/policy-buffer.ts` exporting `PolicyRecordBuffer<T>`; refactor the five to compose it (session `topRefactors` #1, eliminating ~400 LOC, makes the bug surface 1 instead of 5).
- The 7-step action handler pattern (~50 sites): extract `src/tools/action-tool.ts` exporting an `actionTool()` wrapper (tools-and-seam `topRefactors` "extract boilerplate wrappers", a ~50 % code cut on the action family).
- Egress masking (URL-sanitiser + secrets) is hand-called at each sink. Introduce `src/util/egress-sanitiser.ts` exporting `EgressSanitiser`, injected once into every output path (policy-util `topRefactors` "create EgressSanitiser abstraction", which turns caller discipline into a compile-time chokepoint; closes the page-features gap "no guardrail prevents returning `NetworkBuffer.iter()` without masking").
...
- `docs/ai-context/architecture/repo-map.md` and the RFC index: corrected for the stale references the audit found (the coupling-audit reference still cites `server.ts` at 12,889 lines; it is now 382, because the composition-root refactor already landed).
...
**Why this order.** Last because it is the *capstone*: the assertion/bounded-resource pass touches modules that P1–P4 have already settled (asserting invariants on a registry that does not exist yet is premature), and the doc landing must describe the *final* state of all guardrails, and `fitness-functions.md` is only accurate once P0–P4's checks are all in place and promoted.
...
### 4.1 Per-phase definition of done
...
### 4.2 Overall definition of done
...
- [`0004-architecture-hardening.md`](../0004-architecture-hardening.md) is the parent RFC: thesis, the ten laws L1–L10, decisions D1–D12, and §6 phasing this document expands.
