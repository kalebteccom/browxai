...
god-module, an `if (engine === "literal")` leak, a hand-edited central list. The
analogous mechanization is the **fitness function**. We are not writing flight
...
| **5** | Minimum two assertions per function; assertions must be side-effect-free and check anomalous conditions. | **L8 (assert the invariants).** browxai introduces an `invariant()` helper and an assertion-density check on the load-bearing modules; a violated invariant surfaces as a **structured refusal**, never a crash (fault containment, §1.4 below). | **Relaxed on the density floor.** We do not impose "≥2 everywhere", which would manufacture noise assertions in pure data-shaping code. The density check targets the *load-bearing* modules (the session factories, the gate, the substrates), not every utility. |
| **6** | Declare data objects at the smallest possible scope. | **L3 / L4 (segregated contracts).** The TypeScript analogue is the `SessionEntry` (40 fields, `src/session/registry.ts:48`) and `ToolHost` (35 members, `src/tools/host.ts:54-189`) segregation: a consumer depends on the narrow role-bundle it uses, not the god-object: smallest-scope *contract*, not just smallest-scope *variable*. | Lifted from variable scope to interface scope (ISP). |
...
layered ruleset, with rules grouped by severity / risk level, where **each rule is tied
...
- *DO-178C / ED-12C, "Software Considerations in Airborne Systems and Equipment
  ...
  and every line of code and every test traces back up to a requirement. At the
...
2. **Deviations go through a documented process, never an inline silence.** This is
   already browxai norm, in code-quality.md's zero-ignores discipline ("no
...
undefined behavior, none of which touch TypeScript). The two custom rules browxai already
...
audit found the doctrine is *right* and *unenforced*, and these are the sources that
...
exposing 40 fields to every consumer.
...
### 2.5 Cockburn on hexagonal architecture (ports and adapters)
...
- *Holistic* fitness functions (multiple dimensions together) → the
  ...
  core edits, since it exercises the registry, the gate, the substrates, and the
...
- *dependency-cruiser* (sverweij/dependency-cruiser, npm) is the JS/TypeScript
...
mature, industrial practice and not a browxai invention, and as the conceptual model
...
shrinks counterexamples). *fast-check* (dubzzz/fast-check, npm) is the modern
...
(stryker-mutator.io) is the JavaScript/TypeScript mutation-testing framework.
...
| **typescript-eslint type-aware rules** | The `no-unsafe-*` family, `no-explicit-any`, `no-floating-promises`, etc., for boundary type-safety. | **L6** validate-at-the-edge; the five `no-unsafe-*` rules now at `error`. | **Present and at `error`**: the one piece already landed; L6's enforcer exists. |
...
(D10, the DIP ratchet), the **engine-adapter-contract keystone** (L1, which proves the OCP
claim the doctrine *makes but does not verify*), and the **completeness fitness tests**
(L2, closing four OCP findings at once by deriving the central maps).
...
2. **The module decomposition RFC 0003 produced is explicitly flagged as interim.**
   ...
   line-range decomposition, explicitly flagged as thematically loose at the time", and
...
section (the audit found the guardrails are "not consolidated or made visible", since the
...
- [`0004-architecture-hardening.md`](../0004-architecture-hardening.md) is the parent
