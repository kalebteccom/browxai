...
**Tool inventory method:** 167 direct `register("name", …)` calls in `src/server.ts` plus loop-registered families (`mouse_*` ×3 at server.ts:3688, `touch_*` ×3 at :3783, `profile_snapshot|restore` ×2 at :6157, `emulate_{bluetooth,usb,hid}` ×3 at :10529-10539, `{local,session}storage_*` ×10 at :7777) = **198 registered MCP tools**. Cross-checked against `TOOL_CAPABILITY` in `src/util/capabilities.ts` (181 mapped entries; the 17 unmapped: `open_session`, `close_session(s)`, `list_sessions`, `batch`, `get/set/reset_config`, `approve_actions`, `list_approvals`, `plugins_list/info`, `workers_list`, `worker_message*`, `sw_*intercept_fetch`, all default to `human`).
...
Classes:
- **[A]** marks a tool that is engine-agnostic via Playwright's cross-browser API surface, so it would work on `playwright-firefox`/`webkit` today, modulo the launch path and the envelope caveat below.
- **[B]** marks a Chromium/CDP-hard tool that uses `CDPSession`, `connectOverCDP` or a CDP-only Playwright API, with the exact API named in the row.
- **[C]** marks an adaptable tool that works via a different mechanism per engine, where a Playwright-portable replacement or a BiDi module exists and the per-engine work is real.
- **[D]** marks a tool that is conceptually impossible or moot on some engine, named with the reason.
...
|---|---|---|---|
| `touch_start/move/end` | **C** | CDP `Input.dispatchTouchEvent` with `touchPoints[].id` for multi-finger (`gestures.ts:163-200`) | Playwright cross-browser surface has only `touchscreen.tap`. Firefox: WebDriver BiDi `input.performActions` with pointerType `touch` supports multi-pointer, so it needs a raw-BiDi escape hatch or upstream Playwright support. WebKit: no public touch-sequence injection via Playwright → near-**D** on Safari today |
...
|---|---|---|---|
| `perf_start`, `perf_stop` | **B** | `Tracing.start`/`Tracing.end` + `Tracing.dataCollected` (`perf.ts:140-194`) | Chrome trace-event format is engine-specific. Firefox has the Gecko profiler (different protocol, different format), so a Firefox lane is a **separate implementation**, not an adaptation → treat as **D** for v1 multi-engine |
...
| `fs_picker_respond`, `set_fs_picker_policy` | **A**-moot | exposeBinding + init-script stubs (`fs-picker.ts:545,605`); the File System Access API is Chromium-only platform API, so off-Chromium: moot |
...
- `clock`/`network_emulate`/`cpu_emulate` re-apply on `framenavigated` because "CDP may drop overrides on a renderer swap" (`emulation.ts:7-11`, `clock.ts:16-18`): the re-apply machinery is reusable; the appliers are the per-engine part.
...
5. **`src/page/gestures.ts` touch half**: a BiDi `input.performActions` adapter for Firefox; structured refusal on WebKit.
