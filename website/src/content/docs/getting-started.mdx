...
description: Install browxai, connect it to your harness, and run a first agent flow against a real page.
---

import HarnessTabs from "../../components/harness-tabs.astro";

browxai is an [MCP](https://modelcontextprotocol.io/) server that gives an AI
agent a curated browser-control surface. Local harnesses can launch it over
stdio. Cloud harnesses need a remote MCP relay that can reach a browxai host and
browser running on your hardware.
...
stdio transport. `browxai browser install` installs the browser revision pinned
to the installed browxai package.

## Connect it to a harness
