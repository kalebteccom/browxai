# Spike runbook — *optional reference*, no longer a gate

> **Status (2026-05-13):** demoted to optional reference. The original plan was for this
> spike's curated-vs-raw A/B to produce a Phase-0 go/no-go; site-docs (the first consumer)
> filed its six asks on the same day, formally committing to adopt browxai, and the real
> evaluation moved into **Phase 1** as the site-docs adoption run on a real authed target.
> Phase 0 closed without this spike's numbers. The harness is kept because it's a working
> two-surface MCP server you can drive for a sanity-check if useful. **The canonical next
> task is in `../AGENT-RUNBOOK.md` (root of this repo), not here.**

> Hand this doc to an MCP-capable coding agent (Claude Code, Codex, anything that drives MCP
> tools) only if you specifically want to exercise the spike. It explains what the spike is,
> how to install the throwaway MCP server, how to run the two tasks against both surfaces,
...
browxai is an MCP-native, agentic-first browser-control server. The canonical Phase-1
implementation work (build out the curated surface in `src/`, deliver the first-consumer
asks #1–#6, drive site-docs's real adoption run) lives in the top-level `AGENT-RUNBOOK.md`
— that's almost certainly the task you want. This **spike** is a throwaway MCP server in
`spike/` that exposes **both** a raw and a curated surface (selected by an env var at
startup), so you can A/B them on two public-site tasks. Originally meant to be the Phase-0
gate; now optional reference. Run it if a feel for what each surface costs in
tool-calls / failed-calls / "retry indicator" before touching the canonical server is useful
— skip it if you'd rather just go build.
...
|---|---|---|---|
| 1 | `raw`     | `task01` | [`tasks/task01-wikipedia.md`](tasks/task01-wikipedia.md) |
| 2 | `curated` | `task01` | same |
| 3 | `raw`     | `task02` | [`tasks/task02-the-internet.md`](tasks/task02-the-internet.md) |
...
2. Open the task file (`tasks/taskNN-*.md`). It describes the goal, the steps, and
...
- **The spike is throwaway.** Do not refactor `server.ts` or `browser.ts` mid-run
  to make a tool nicer. If a tool is awkward, *that's data* — note it in the verdict.
- **Don't change task wording mid-run.** The tasks live in `tasks/` for a reason — if
