...
It drives six engines behind one tool surface: Chromium, Firefox and WebKit as browsers it launches for you, real Safari.app over `safaridriver`, real Chrome on an Android handset attached over adb, and a running desktop Electron app (VS Code, Slack, Discord) attached over its `--remote-debugging-port`. Android and Electron are attach-only, so the browser or app has to already be running; browxai never spawns one. Attaching to a signed-in desktop app has a security posture worth reading before you do it — see [`docs/threat-model.md`](docs/threat-model.md). There is no native-app automation and no iOS. CI runs the cross-engine suite on Chromium, Firefox and WebKit on every commit; Android needs a USB device and Safari needs macOS, so those two are exercised by hand.
