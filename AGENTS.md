...
- `deploy/` — reference deployment artifacts for adopters (`Dockerfile`). Keep the Playwright base-image tag aligned with the `playwright-core` range in `package.json`.
...
| OFF + loud-warn | `native-device`    | gates an ENGINE, not a tool: `ios-app` refuses at session creation without it                |
...
- **`docs/ai-context/`** — agent-facing routing layer. Discipline + architecture notes + lessons. **NOT published.** Read before touching the relevant area. Subtree IA: `agent-process/`, `architecture/`, `tool-registration/`, `page-side-functions/`, `recorder-and-replay/`, `plugin-runtime/`, `secrets-and-egress/`, `testing/`, `release-process/`, `adopter-reports/`, `investigations/`. The macro-layer architecture doctrine (dependency direction, proven-seam test, performance budgets) is `architecture/architecture-principles.md` — read it with `agent-process/code-quality.md` for any boundary, world-touching-surface, or hot-path change.
