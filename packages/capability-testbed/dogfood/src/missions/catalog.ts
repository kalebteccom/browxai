...
export const CATALOG: readonly DogfoodMission[] = [
  ...
  mission({
    id: "multi-engine-sessions",
    capabilityTags: ["read", "navigation", "control"],
    surfaces: ["core"],
    goal: [
      "Exercise PER-SESSION browser ENGINE selection on a SINGLE browxai server (the server defaults to chromium).",
      "Your already-loaded session uses the default engine — call list_sessions and note its `engine`.",
      'Then, in the SAME server: (1) open a new session with open_session({session:"ff", engine:"firefox", mode:"incognito"}) and one with open_session({session:"wk", engine:"webkit", mode:"incognito"}); note the `engine` each open_session result reports.',
      "(2) Navigate each new session to the test app base URL and take a snapshot, to prove the engine actually drove a real page.",
      "(3) Call list_sessions and confirm chromium, firefox, and webkit sessions COEXIST at once in one server.",
      "(4) On the firefox session try a CDP-deep tool such as perf_start and record the refusal; run perf_start on the chromium session too and note the difference (the gate is per-session).",
      '(5) Probe the error UX: try open_session({session:"droid", engine:"android"}) (no device is attached) and open_session({session:"nope", engine:"opera"}) (not a real engine) — record exactly what each returns.',
      "Finish with a concise report on how the per-session engine feature FELT: were the engine names and constraints discoverable from the tool schema, were the refusals/errors clear and actionable, and did anything confuse you or force guessing?",
    ].join(" "),
    sourceFiles: ["read-core.ts", "navigation.ts", "control.ts"],
    tools: [...surfaceTools("/core", ["read", "navigation"]), ...rootTools()],
    kRuns: 1,
  }),
  mission({
