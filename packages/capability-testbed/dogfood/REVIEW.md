...
PASS. I created `.write-probe` at the repository root with content `OK`, read it back successfully, and deleted it before this review file was written.
...
8. Use the Codex reasoning stream in friction metrics, but label it accurately. The app-server can emit reasoning summaries in `../remotxai/adapters/codex/src/app-server.ts:250` and the adapter advertises `reports_reasoning` in `../remotxai/adapters/codex/src/index.ts:118`, but this is not guaranteed to be full hidden thinking tokens. Metrics should measure visible reasoning summaries and token usage, not claim direct access to private chain-of-thought.
...
The core host-run path is viable. `CodexAppServerOwn` is host-owned: `defaultSpawn` calls `codex app-server` in `../remotxai/adapters/codex/src/app-server-own.ts:108`, and the constructor invokes that spawn before sending `initialize` in `../remotxai/adapters/codex/src/app-server-own.ts:211`. The sandbox posture is a thread parameter, not the process sandbox for the host runner, because `threadStartParams()` serializes it into the `thread/start` request in `../remotxai/adapters/codex/src/app-server-own.ts:466`.
...
The design correctly avoids `codex app-server --listen` raw JSON because that path is known to fail. The remotxai adapter documents the exact error as `failed to upgrade control socket websocket connection: WebSocket protocol error: httparse error: invalid token` in `../remotxai/adapters/codex/src/app-server.ts:10`.

The package dependency claim is grounded: the local remotxai adapter is private in `../remotxai/adapters/codex/package.json:4`, and the design says not to depend on it in `packages/capability-testbed/dogfood/DESIGN.md:14`. The fallback is architecturally sufficient but operationally incomplete until it has fixtures and drift tests.

### Axis 2 — Ergonomic Feel Capture

The event stream can include reasoning summaries, not just tool names. The design normalizes `item/completed` reasoning in `packages/capability-testbed/dogfood/DESIGN.md:207`; the remotxai app-server maps completed reasoning items in `../remotxai/adapters/codex/src/app-server.ts:250`; and the adapter declares `reports_reasoning: true` in `../remotxai/adapters/codex/src/index.ts:118`.
