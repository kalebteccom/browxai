  ...
  register(
    "set_fs_picker_policy",
    {
      ...
      description:
        ...
        '  - "ask-human" — server blocks on `__browx.respond({kind:"fs_picker_respond", value:{files:[…]}})` with the ticket printed in the prompt, from the DevTools `browxai-…` console context it names (the `await_human` mechanism; page scripts cannot answer it), then resolves with the human-approved file list or denies.\n' +
