// ActionResult type surface — the engine-blind vocabulary the action window
// produces and consumes. Split out of actionresult.ts to keep that file under
// the size budget; re-exported from `./actionresult.js` so callers import
// unchanged.

// Every import here is a vendor-free leaf, and that is load-bearing: the
// ActionSubstrate port names `ActionResult` in all twelve of its methods, so
// anything this module reaches the port reaches too. The Playwright-shaped
// half — `ActionContext`, which carries the `Page` — lives in
// `action-context.ts`. (RFC 0009 P1.)
import type { NetworkEntry, NetworkSummary, MutationEntry, WsFrame } from "./network-types.js";
import type { SnapshotMode } from "./actionresult-shape.js";
import type { DialogRecord } from "../session/dialog-policy.js";
import type { PermissionRecord } from "../session/permission-policy.js";
import type { NotificationRecord } from "../session/notification-policy.js";
import type { FsPickerRecord } from "../session/fs-picker-policy.js";
