// .heapsnapshot on-disk + parsing — the engine-blind side of heap tooling.
//
// CDP hands the snapshot back as a stream of chunks; the chromium ecosystem
// (DevTools Memory panel, `chrome://inspect`) reads/writes that as a single
// `.heapsnapshot` JSON document. This module owns that serialization contract
// plus the workspace-rooted path guard AND the structural parse + retainer
// query — all engine-blind by design, so the CDP snapshot lifecycle in
// `heap.ts` and the diff tool in `memory-diff.ts` both share one definition of
// "what a snapshot file is", "where it's allowed to live", and "how to walk
// it". Keeping it here (not in heap.ts) means the path guard / file shape /
// parse can change without touching the CDP lifecycle, and `memory-diff.ts`
// can consume `parseHeapSnapshot` without dragging in CDP state.
//
