// SafariStorageSubstrate — the StorageSubstrate implementation for the real-Safari
// engine. Safari has no Playwright BrowserContext/Page, so it rides WebDriver
// Classic instead: cookies over the `getCookies` / `addCookie` endpoints, and
// web-storage over the `execute/sync` endpoint (page-side JS, which safaridriver
// CAN run — the same `return (…)` expression wrapping the ScriptSubstrate uses).
// For cookies, safaridriver scopes the jar to the current document, so the `urls`
// filter is inert (the WebDriver protocol has no cross-domain cookie filter) and
// `cookies_set` derives the domain from `url` exactly as the pre-seam Safari branch
// did. For web-storage the origin guard reads `currentUrl()` — the Classic
// substitute for the Playwright `page.url()` the helper guards on. The gating lives
...
// (the port in `storage-substrate-types.ts`) → this implementation → safaridriver.
// The port + result types live in the leaf `storage-substrate-types.ts`, which both
// impls import; this file never imports back from the `storage-substrate.js` barrel
// that re-exports it.
