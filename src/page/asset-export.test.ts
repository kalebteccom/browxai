...
describe("assetExport integration", () => {
  it("filters by mime + urlPattern, persists, writes _manifest.json", async () => {
    ...
    try {
      ...
      const r = await assetExport({ cdp, page, buffer: buf, workspaceRoot: root, sessionId: "sess" }, args);
  ...
  it("collision-resolves identical basenames from different paths", async () => {
    ...
    try {
      ...
      const r = await assetExport({ cdp, page, buffer: buf, workspaceRoot: root, sessionId: "sess" }, { filter: {} });
  ...
  it("respects maxCount cap and reports it in warnings", async () => {
    ...
    try {
      ...
      const cdp = stubCdpNoBody();
      const page = stubPageWith(() => ({ ok: true, base64: Buffer.from("x").toString("base64") }));
      const r = await assetExport({ cdp, page, buffer: buf, workspaceRoot: root, sessionId: "sess" }, { filter: {}, maxCount: 2 });
  ...
  it("respects maxBytes cap and stops the export early", async () => {
    ...
    try {
      ...
      const r = await assetExport({ cdp, page, buffer: buf, workspaceRoot: root, sessionId: "sess" }, { filter: {}, maxBytes: 10 });
      expect(r.persistedCount).toBe(1);
  ...
  it("clamps caller-supplied maxCount to the hard ceiling", async () => {
    ...
    try {
      ...
      const r = await assetExport({ cdp, page, buffer: buf, workspaceRoot: root, sessionId: "sess" }, {
        filter: {},
