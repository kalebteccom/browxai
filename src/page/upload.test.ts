...
describe("uploadFile", () => {
  ...
  it("rejects a path that escapes the workspace", async () => {
    const { page } = fakePage();
    await expect(
      uploadFile(page as never, refs, WS, { target: { selector: "#f" } as never, path: "../../etc/passwd" }),
  ...
  it("rejects an absolute path outside the workspace", async () => {
    const { page } = fakePage();
    await expect(
      uploadFile(page as never, refs, WS, { target: { selector: "#f" } as never, path: "/etc/hosts" }),
    ).rejects.toThrow(/inside \$BROWX_WORKSPACE/);
  });
  ...
  it("rejects passing both content and path", async () => {
    const { page } = fakePage();
    await expect(
      uploadFile(page as never, refs, WS, { target: { selector: "#f" } as never, content: "AA==", path: "x" }),
