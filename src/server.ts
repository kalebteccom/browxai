  ...
  register(
    "extract",
    {
      description:
        ...
        "**Supported `type` values (closed set):** `object`, `array`, `string`, `number`, `boolean`. JSON-Schema's `integer` is NOT supported — use `\"number\"` (the leaf coerces numeric strings, so an integer-shaped page value still arrives as a JS number). `null`, `any`, and union types are also rejected. " +
        "Deterministic by default: each property lowers to a selector-based query scoped to the current subtree. **Implicit rule**: the property *name* IS the find()-style query — `{type:\"string\"}` property \"price\" matches a node whose accessible name / testid contains \"price\". **Explicit escape hatch — `x-browx-source` per property** with one of these keys (other keys are silently dropped today but surface as `unknown \\`x-browx-source\\` key` diagnostics in `evidence.partialMisses`): `selector` (raw CSS, scoped to current locator), `query` (NL find()-style), `attr` (HTML attribute name — NOT `attribute`), `prop` (DOM property name — NOT `property`), `text:true` (visible-text, the default), `value:true` (form-control value, alias for `prop:\"value\"`). No `transform`/`format`/`regex` — the leaf coercer handles `\"$1,234.50\" → 1234.5` for `type:\"number\"` automatically. " +
        "**For lists**: `{type:\"array\", items:<schema>, \"x-browx-source\":{collection:\"<selectorOrQuery>\"}}` — `collection` is REQUIRED on every array (the row-container CSS selector or NL query; each match becomes a per-row scope for `items`). Arrays without `collection` are surfaced as a partialMiss (or required-miss failure if `required:true`); there's no defensible implicit default. " +
