...
import { resolveCapabilities, resolveConfirmHooks, isToolEnabled, TOOL_CAPABILITY, type Capability } from "./util/capabilities.js";
...
export async function createServer(opts: StartOptions = {}): Promise<{
  ...
  register(
    "extract",
    {
      ...
      inputSchema: {
        schema: z.record(z.unknown()).describe("JSON-schema-flavoured shape (object/array/string/number/boolean; `properties` for objects, `items` for arrays). `x-browx-source.selector` (raw CSS) per-property overrides the implicit name-as-query rule. (`x-browx-source.query` is RETIRED in v0.3.3 — tolerated with warn + partialMisses entry.) `required:true` causes a miss to fail-emit; `default` supplies an optional-miss fallback."),
        ...
        mode: z.enum(["deterministic", "llm-assisted"]).optional().describe("RETIRED in v0.3.2. Default and only supported value is 'deterministic' (selector-only). 'llm-assisted' is tolerated for back-compat (warn + fall through to deterministic) but is no longer in the typed SDK surface; drop the arg from new code."),
        ...SESSION_ARG,
      },
    },
    async (args) => {
  ...
  register(
    "screenshot_schedule",
    {
      ...
      inputSchema: {
        everyMs: z.number().int().min(100).max(60_000).describe("Interval between captures (ms). Range [100, 60000]."),
        count: z.number().int().min(1).max(1000).optional().describe("Stop after N captures. Mutually exclusive with `durationMs`."),
        durationMs: z.number().int().positive().optional().describe("Stop after this wall-clock window (ms). Mutually exclusive with `count`. Must be >= `everyMs`."),
        intoDir: z.string().optional().describe("Workspace-rooted output directory. Default `screenshots/<sessionId>-<isoTs>/`. Path-traversal rejected."),
        format: z.enum(["png", "jpeg"]).optional().describe("Image format. Default `png`. `jpeg` files are written with `.jpg` extension."),
        ...SESSION_ARG,
      },
    },
    async (args) => {
      const g = gateCheck("screenshot_schedule"); if (g) return g;
      try {
        const e = await entryFor(args.session);
        const page = e.session.page();
        const fmt: "png" | "jpeg" = args.format ?? "png";
  ...
  register(
    "screenshot_on",
    {
      ...
      inputSchema: {
        trigger: z.enum(["navigation", "console-error", "network-mutation", "dialog"])
        ...
        durationMs: z.number().int().min(1).max(600_000).describe("Observation window length (ms). Range [1, 600000] (10 min ceiling)."),
        intoDir: z.string().optional().describe("Workspace-rooted output directory. Default `screenshots/<sessionId>-<isoTs>/`. Path-traversal rejected."),
        format: z.enum(["png", "jpeg"]).optional().describe("Image format. Default `png`. `jpeg` files are written with `.jpg` extension."),
        ...SESSION_ARG,
      },
    ...
    async (args) => {
      const g = gateCheck("screenshot_on"); if (g) return g;
      try {
        const e = await entryFor(args.session);
        const page = e.session.page();
        ...
        const source = {
          subscribe: (
            ...
            return () => { for (const d of disposers) { try { d(); } catch { /* listener already gone */ } } };
          },
        };

        const result = await withDeadline(
          runScreenshotOn(snap, source, {
  ...
  register(
    "workers_list",
    {
      ...
      inputSchema: {
        type: z.enum(["web", "service", "all"]).optional().describe("Filter by worker type. Default `\"all\"`."),
        ...SESSION_ARG,
      },
    },
    async ({ type, session }) => {
      const g = gateCheck("workers_list"); if (g) return g;
      const e = await entryFor(session);
      try {
  ...
  register(
    "worker_message_send",
    {
      ...
      inputSchema: {
        ...
        message: z.string().describe("Payload to send. Strings only — structured-clone / `MessagePort` transfer not supported in MVP."),
        ...SESSION_ARG,
      },
    },
    async ({ workerId, message, session }) => {
      const g = gateCheck("worker_message_send"); if (g) return g;
      const e = await entryFor(session);
      try {
        const r = await e.workers.sendMessage(e.session.page(), e.session.cdp(), { workerId, message });
        const body = { ...r, tokensEstimate: 0 };
        body.tokensEstimate = estimateTokens(JSON.stringify(body));
        return { content: [{ type: "text" as const, text: JSON.stringify(body, null, 2) }] };
      } catch (err) {
        const body = { ok: false, workerId, error: err instanceof Error ? err.message : String(err), tokensEstimate: 0 };
        body.tokensEstimate = estimateTokens(JSON.stringify(body));
        return { content: [{ type: "text" as const, text: JSON.stringify(body, null, 2) }] };
      }
    },
  ...
  register(
    "worker_messages_read",
    {
      ...
      inputSchema: {
        workerId: z.string().optional().describe("Drain only this worker's messages. Omit to drain ALL workers."),
        ...SESSION_ARG,
      },
    },
    async ({ workerId, session }) => {
      const g = gateCheck("worker_messages_read"); if (g) return g;
      const e = await entryFor(session);
      try {
  ...
  register(
    "sw_intercept_fetch",
    {
      ...
      inputSchema: {
        pattern: z.string().describe("Glob matched against the intercepted request URL, e.g. `https://api.example/**`."),
        response: z.object({
    ...
    async ({ pattern, response, session }) => {
      const g = gateCheck("sw_intercept_fetch"); if (g) return g;
      const e = await entryFor(session);
      try {
  ...
  register(
    "sw_unintercept_fetch",
    {
      ...
      inputSchema: {
        pattern: z.string().optional().describe("Omit to clear ALL of this session's SW fetch interceptors."),
        ...SESSION_ARG,
      },
    },
    async ({ pattern, session }) => {
      const g = gateCheck("sw_unintercept_fetch"); if (g) return g;
      const e = await entryFor(session);
      try {
        const r = await e.workers.removeFetchIntercept(e.session.cdp(), pattern !== undefined ? { pattern } : {});
        const body = { ok: true, ...r, tokensEstimate: 0 };
        body.tokensEstimate = estimateTokens(JSON.stringify(body));
        return { content: [{ type: "text" as const, text: JSON.stringify(body, null, 2) }] };
      } catch (err) {
  ...
  register(
    "canvas_capture",
    {
      ...
      inputSchema: {
        ref: z.string().optional().describe("Stable [eN] ref of the canvas element. Omit to use the first `<canvas>` (with a `selector` fallback when supplied)."),
        selector: z.string().optional().describe("Fallback CSS selector path used when `ref` does not resolve."),
        format: z.enum(["png", "webgl-framebuffer", "2d-imagedata"]).describe("`png` → base64 PNG bytes (handoff to vision); `2d-imagedata` → base64 RGBA bytes (pixel math); `webgl-framebuffer` → base64 RGBA from `gl.readPixels`, flipped to top-left to match imagedata convention."),
        ...SESSION_ARG,
      },
    },
    async (args) => {
      const g = gateCheck("canvas_capture"); if (g) return g;
      const e = await entryFor(args.session);
      try {
        const r = await withDeadline(
  ...
  register(
    "canvas_diff",
    {
      ...
      inputSchema: {
        beforeBase64: z.string().describe("Base64 RGBA bytes (or PNG when `inputFormat:'png'`) from a prior capture."),
        afterBase64: z.string().describe("Base64 RGBA bytes (or PNG) from a later capture."),
        width: z.number().int().positive().optional().describe("Pixel width of the captures. Required for RGBA inputs."),
        height: z.number().int().positive().optional().describe("Pixel height of the captures. Required for RGBA inputs."),
        region: z.object({
          x: z.number(),
          y: z.number(),
          w: z.number(),
          h: z.number(),
        }).optional().describe("Optional sub-rectangle (image px, top-left origin)."),
        inputFormat: z.enum(["rgba", "png"]).optional().describe("Defaults `rgba`. Pass `png` for PNG-format inputs (this cycle: base64 byte equality only + warning)."),
        ...SESSION_ARG,
      },
    },
    async (args) => {
      const g = gateCheck("canvas_diff"); if (g) return g;
      try {
        const r = canvasDiff({
  ...
  register(
    ...
    async (args) => {
      const g = gateCheck("gesture_chain"); if (g) return g;
      const e = await entryFor(args.session);
      try {
        const r = await withDeadline(
  ...
  register(
    "canvas_world_to_screen",
    {
      ...
      inputSchema: {
        ...
        ref: z.string().optional().describe("Stable canvas ref. Not used for math today; reserved for adapter dispatch."),
        selector: z.string().optional().describe("Canvas selector path. Not used for math today; reserved for adapter dispatch."),
        transform: z.object({
          scale: z.number(),
          panX: z.number(),
          panY: z.number(),
        ...
        }).optional().describe("Explicit transform. Omit to trigger heuristic discovery."),
        ...SESSION_ARG,
      },
    },
    async (args) => {
      const g = gateCheck("canvas_world_to_screen"); if (g) return g;
      const e = await entryFor(args.session);
      try {
        const r = await withDeadline(
  ...
  register(
    "canvas_screen_to_world",
    {
      ...
      inputSchema: {
        ...
        selector: z.string().optional().describe("Canvas selector path. Reserved for adapter dispatch."),
        transform: z.object({
          scale: z.number(),
          panX: z.number(),
          panY: z.number(),
    ...
    async (args) => {
      const g = gateCheck("canvas_screen_to_world"); if (g) return g;
      const e = await entryFor(args.session);
      try {
        const r = await withDeadline(
  ...
  register(
    "canvas_query",
    {
      ...
      inputSchema: {
        ...
        op: z.string().describe("Operation name under the plugin's namespace — combined as `<adapter>.<op>` for the registry lookup."),
        args: z.record(z.unknown()).optional().describe("Forwarded as the inner tool's args."),
        ...SESSION_ARG,
      },
    },
    async (args) => {
      const g = gateCheck("canvas_query"); if (g) return g;
      ...
      const inner = await fn({ ...(args.args ?? {}), ...(args.session !== undefined ? { session: args.session } : {}) });
      return inner;
    },
  );
  ...
  const FILL_FORM_FIELD = z.object({
    ...
    value: z.string().describe("Value to fill (substring `<NAME>` triggers secrets materialisation when the secrets capability is on)"),
  });
  ...
  const fieldArgToTarget = (
    ...
    if (provided === 0) throw new Error(`fill_form: ${label} requires one of \`ref\` / \`selector\` / \`named\``);
    if (provided > 1) throw new Error(`fill_form: ${label} — pass exactly one of \`ref\` / \`selector\` / \`named\``);
    if (raw.ref) return { ref: raw.ref };
    if (raw.named) {
      const resolved = refs.refByNameLookup(raw.named);
      if (!resolved) throw new Error(`fill_form: ${label} — name "${raw.named}" not bound. Call name_ref({name, ref}) first.`);
      return { ref: resolved };
    }
  ...
  register(
    "fill_form",
    {
      ...
      inputSchema: {
        fields: z.array(FILL_FORM_FIELD).min(1).describe("Ordered list of {target, value} pairs. Filled sequentially after atomic pre-resolution."),
    ...
    async (args) => {
      const g = gateCheck("fill_form"); if (g) return g;
      const e = await entryFor(args.session);
  ...
  const BATCH_ALLOWED_TOOLS = new Set<string>([
    "navigate", "click", "fill", "fill_form", "press", "hover", "select", "choose_option", "wait_for",
  ...
  register(
    "plugins_info",
    {
      ...
      inputSchema: {
        name: z.string().describe('npm package name of the plugin (e.g. "@kalebtec/browxai-plugin-example").'),
      },
    },
