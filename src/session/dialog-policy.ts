// Per-session dialog policy — the Node-side decision state.
//
// Policy modes:
...
//
// Per-action capture: every fired dialog is appended to a bounded buffer with a
// timestamp. `since(ts)` slices the buffer for the action-window. The
// `raise`-mode flag (`raisedSince(ts)`) lets the action layer convert the action
// to `ok:false` without re-parsing the dialog records.
//
// Split out of `dialog.ts` along the same realm cut the three sibling policies
// already follow (`permission-policy` / `notification-policy` /
// `fs-picker-policy`): decision state here, the Playwright `page.on('dialog')`
// wiring in `dialog-attach.ts`, both re-exported through the `dialog.ts` barrel.
// The cut is what takes playwright-core off the reach of `DialogRecord`, which
// `ActionResult.dialogs[]` names and the ActionSubstrate port therefore reaches.
// (RFC 0009 P1.)
...
export class DialogPolicyState {
  ...
  hasPage(p: object): boolean {
    return this.wired.has(p);
  }
  /** Mark a page as wired. */
  markPage(p: object): void {
    this.wired.mark(p);
  }
}
