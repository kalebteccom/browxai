// Per-session role-bundle TYPE catalogue. The static shape of one isolated
// SessionEntry — its engine-core identity, the ISP role bundles its consumers
// depend on, and the `OpenSpec` creation contract. This is a leaf TYPES file:
// it imports only the per-subsystem state types it composes and is re-exported
// verbatim by `registry.ts` (the barrel), so importers see no path change.
//
// Split from the runtime `SessionRegistry` class on purpose — one reason to
// change: this catalogue grows when a feature field is added; the class changes
// when the session lifecycle changes. Keeping them apart stops a field-add from
// recompiling the lifecycle and vice-versa.
