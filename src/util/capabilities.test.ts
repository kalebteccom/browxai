...
describe("resolveCapabilities (BROWX_CAPABILITIES)", () => {
  ...
  it("rejects unknown capabilities loudly", () => {
    expect(() =>
      resolveCapabilities({ BROWX_CAPABILITIES: "read,banana" } as NodeJS.ProcessEnv),
    ).toThrow(/banana/);
  });
  ...
  it("a genuine typo still throws even alongside a retired capability", () => {
    expect(() =>
      resolveCapabilities({ BROWX_CAPABILITIES: "read,unstable,banana" } as NodeJS.ProcessEnv),
    ).toThrow(/banana/);
  });

  it("emits no warnings for an all-valid capability set", () => {
    expect(
      resolveCapabilities({ BROWX_CAPABILITIES: "read,navigation" } as NodeJS.ProcessEnv).warnings,
    ).toEqual([]);
  });
...
describe("resolveConfirmHooks (BROWX_CONFIRM_REQUIRED)", () => {
  ...
  it("rejects unknown hooks", () => {
    expect(() =>
      resolveConfirmHooks({ BROWX_CONFIRM_REQUIRED: "fly_to_moon" } as NodeJS.ProcessEnv),
    ).toThrow(/fly_to_moon/);
  });
});
