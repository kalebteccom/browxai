...
const ALLOWED = new Set([
  "click", "fill", "navigate", "snapshot", "wait_for", "find", "plan", "execute",
]);
...
describe("runFlakeCheck — repetition + roll-up", () => {
  ...
  it("captures per-step success rate when one step flakes", async () => {
    ...
    const click = scriptedHandler([
      { ok: true }, { ok: false, error: "boom" }, { ok: true }, { ok: false, error: "boom" }, { ok: true },
    ]);
  ...
  it("findFirstDivergence returns null when every run agrees per step", () => {
    ...
    const allOk = [
      { completed: 1, failedAt: null, results: [{ tool: "click", ok: true, result: { ok: true } }] },
      { completed: 1, failedAt: null, results: [{ tool: "click", ok: true, result: { ok: true } }] },
    ];
...
describe("runFlakeCheck — stopOnAllGreen short-circuit", () => {
  it("breaks early once stopOnAllGreen consecutive runs are all-green", async () => {
    ...
    const report = await runFlakeCheck(
      [{ tool: "navigate" }],
      { n: 10, stopOnAllGreen: 3, allowed: ALLOWED, handlers: { navigate } },
    );
  ...
  it("does NOT short-circuit when a failure breaks the all-green streak", async () => {
    const click = scriptedHandler([
      { ok: true }, { ok: false, error: "flake" }, { ok: true }, { ok: true },
    ]);
    const report = await runFlakeCheck(
      [{ tool: "click", args: { ref: "e1" } }],
      { n: 4, stopOnAllGreen: 3, allowed: ALLOWED, handlers: { click } },
    );
...
describe("runFlakeCheck — cached-selector artifact", () => {
  it("emits a descriptor-shaped resolver for a `plan` step where every run agreed", async () => {
    const planResp = {
      ok: true,
      descriptor: {
        id: "d-1", ref: "e7", verb: "click", args: {},
        evidence: {
          selectorHint: 'role=button[name="Save"]',
          selectorTier: 2, stability: "medium",
          role: "button", name: "Save", score: 0.9,
          actionable: true, warnings: [],
        },
        expiresAt: Date.now() + 60_000,
      },
    };
    const handlers = { plan: jsonHandler(planResp), execute: jsonHandler({ ok: true, result: { ok: true } }) };
    const report = await runFlakeCheck(
      [
  ...
  it("emits a bound-target resolver for a `click({ref:'eN'})` step that passed every run", async () => {
    ...
    const report = await runFlakeCheck(
      [{ tool: "click", args: { ref: "e3" }, label: "submit" }],
      { n: 3, allowed: ALLOWED, handlers },
    );
  ...
  it("does NOT cache a step that flaked (some runs failed)", async () => {
    ...
    const report = await runFlakeCheck(
      ...
      { n: 3, allowed: ALLOWED, handlers: { click } },
    );
  ...
  it("does NOT cache a `plan` step whose top candidate drifted across runs", async () => {
    ...
    const plan = scriptedHandler([
      { ok: true, descriptor: { ref: "e7", verb: "click", args: {}, evidence: { selectorHint: "role=button[name=Save]" }, expiresAt: 0 } },
      { ok: true, descriptor: { ref: "e9", verb: "click", args: {}, evidence: { selectorHint: "role=button[name=Save (1)]" }, expiresAt: 0 } },
      { ok: true, descriptor: { ref: "e7", verb: "click", args: {}, evidence: { selectorHint: "role=button[name=Save]" }, expiresAt: 0 } },
    ]);
    const report = await runFlakeCheck(
      [{ tool: "plan", args: { query: "save", verb: "click" } }],
      { n: 3, allowed: ALLOWED, handlers: { plan } },
    );
    expect(report.cachedResolvers).toHaveLength(0);
  ...
  it("emits a find-style resolver from a `find` step's top candidate", async () => {
    const findResp = { ok: true, candidates: [{ ref: "e11", selectorHint: 'getByRole("link",{name:"Docs"})', role: "link", score: 0.8 }] };
    const handlers = { find: jsonHandler(findResp) };
    const report = await runFlakeCheck(
      [{ tool: "find", args: { query: "docs" } }],
      { n: 2, allowed: ALLOWED, handlers },
    );
...
describe("runFlakeCheck — internals (pure helpers)", () => {
  it("signatureFor extracts plan ref+hint when present", () => {
    const sig = signatureFor(
      { tool: "plan" },
      { tool: "plan", ok: true, result: { descriptor: { ref: "e1", evidence: { selectorHint: "role=button" } } } },
    );
  ...
  it("signatureFor distinguishes selector vs named targets", () => {
    expect(signatureFor(
      { tool: "click", args: { selector: "#save" } },
      { tool: "click", ok: true },
    )).toBe("selector:#save");
    expect(signatureFor(
      { tool: "click", args: { named: "save-btn" } },
      { tool: "click", ok: true },
    )).toBe("named:save-btn");
  });
...
describe("runFlakeCheck — forces stopOnError:false inside each run", () => {
  it("a failure mid-sequence does NOT halt the inner run — variance is captured for later steps too", async () => {
    const click = scriptedHandler([
      { ok: false, error: "step-1 always fails" }, { ok: false, error: "step-1 always fails" },
    ]);
    const fill = scriptedHandler([{ ok: true }, { ok: true }]);
    const report = await runFlakeCheck(
      [{ tool: "click" }, { tool: "fill" }],
      { n: 2, allowed: ALLOWED, handlers: { click, fill }, stopOnError: true /* should be ignored */ },
    );
  ...
  it("integration smoke — composes cleanly with runBatch's handler dispatch", async () => {
    ...
    const r = await runBatch(
      [{ tool: "navigate" }],
      { allowed: ALLOWED, handlers: { navigate: jsonHandler({ ok: true }) } },
    );
